...
# def train_sam_with_mask_prompt(args, net: nn.Module, optimizer1, optimizer2, train_loader, epoch):
#     """
...
#             # --- MODIFICATION START ---
#             # Extract spacing_info from the batch
#             spacing_info = pack.get('spacing_info', None)
#             if spacing_info is not None:
#                 spacings = spacing_info['spacings_norm'].to(dtype=torch.float32, device=GPUdevice)
#             else:
#                 spacings = None
#             # --- MODIFICATION END ---
...
def train_sam_with_mask_prompt(args, net: nn.Module, optimizer1, optimizer2, train_loader, epoch):
    """
    Train the SAM model using mask prompts with spacing-aware attention
    """
    epoch_loss = 0
    epoch_prompt_loss = 0
    epoch_non_prompt_loss = 0
    ...
    with tqdm(total=len(train_loader), desc=f'Epoch {epoch}', unit='img') as pbar:
        for batch_idx, pack in enumerate(train_loader):
            ...
            # --- SIMPLIFIED SPACING HANDLING ---
            # Extract spacing info from your dataset (single number per sample)
            spacing_info = pack.get('spacing_info', None)
            spacings = None

            if spacing_info is not None:
                # Your dataset provides a single spacing value
                single_spacing = spacing_info.get('spacings_norm', None)  # or however it's named in your pkl

                if single_spacing is not None:
                    # Convert single spacing to tensor for consecutive slices
                    num_slices = min(video_length, len(imgs_tensor.squeeze(0)))
                    if num_slices > 1:
                        # Create uniform spacing tensor: [spacing, spacing, spacing, ...]
                        if isinstance(single_spacing, (int, float)):
                            spacings = torch.full((num_slices - 1,), single_spacing,
                                                  dtype=torch.float32, device=GPUdevice)
                        else:  # if it's already a tensor but scalar
                            spacings = torch.full((num_slices - 1,), single_spacing.item(),
                                                  dtype=torch.float32, device=GPUdevice)

            # If no spacing info available, could set default or leave as None
            # spacings = None means no spacing-aware attention (fallback to original SAM2)
            # --- END SIMPLIFIED SPACING HANDLING ---
            ...
            # Initialize inference state with spacing information
            train_state = net.train_init_state(imgs_tensor=imgs_tensor)

            # Store spacing information in inference state
            if spacings is not None:
                train_state['spacings'] = spacings
