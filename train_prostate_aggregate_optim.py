...
import cfg
from func_3d import function_gland_boxes_aggregate_optim as function
...
def main():
    ...
    sam_layers = (
            ...
            + list(net.sam_prompt_encoder.parameters()) ## not from med sam 2
            + lora_params
    )
    mem_layers = (
            []
    ...
    trainable_params = sam_layers + mem_layers
    optimizer = optim.Adam(trainable_params, lr=1e-5, betas=(0.9, 0.999), eps=1e-08, weight_decay=0, amsgrad=False)
    # if len(sam_layers) == 0:
    #     optimizer1 = None
    # else:
    ...
    for epoch in range(settings.EPOCH):
        ...
        loss, prompt_loss, non_prompt_loss = function.train_sam_with_mask_prompt(
            args, net, optimizer, train_loader, epoch
        )
