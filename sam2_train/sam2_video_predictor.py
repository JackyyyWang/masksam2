...
class SAM2VideoPredictor(SAM2Base):
    ...
    def _run_single_frame_inference(
        ...
        # --- MEDICAL SPACING MODIFICATION START ---
        # Retrieve spacings from the inference state (already set by dataset)
        spacings = inference_state.get('spacings')
        if spacings is not None:
            # Ensure spacings tensor is on the correct device
            device = current_vision_feats[-1].device
            spacings = spacings.to(device=device, non_blocking=True)
        # --- MEDICAL SPACING MODIFICATION END ---
        ...
        current_out = self.track_step(
            ...
            prev_sam_mask_logits=prev_sam_mask_logits,
            spacings=spacings,  # Pass spacings from dataset
        )
    ...
    #
    #     # --- MODIFICATION START ---
    #     # Retrieve spacings from the inference state and ensure correct device
    #     spacings = inference_state.get('spacings')
    #     if spacings is not None:
    #         device = current_vision_feats[-1].device
    #         spacings = spacings.to(device=device, non_blocking=True)
