...
class RankAllocator(object):
    ...
    def standardize_param_name(self, name, module=None):
        """Standardize parameter name to avoid inconsistencies."""
        # Remove any 'image_encoder.' prefix if present
        if name.startswith("image_encoder."):
            name = name[len("image_encoder."):]
    ...
    def calculate_score(self, n, p=None, metric="ipt"):
        ...
        n_std = self.standardize_param_name(n)

        # Check if the parameter is tracked
        if n_std not in self.exp_avg_ipt:
            print(f"Warning: {n_std} not found in tracking dictionaries")
            # Try to find it with other prefixes
            found = False
            for key in self.exp_avg_ipt.keys():
                if key.endswith(n_std.split(".")[-1]):
                    n_std = key
                    found = True
                    break

            if not found:
                print(f"Error: Cannot find {n} in tracking dictionaries")
                # Return zeros to avoid crashing
                if p is not None:
                    return torch.zeros_like(p)
                else:
                    # Try to get the parameter from the model
                    for name, param in self.model.named_parameters():
                        if name.endswith(n_std.split(".")[-1]):
        ...
        if metric == "ipt":
            # Combine the sensitivity and uncertainty
            ipt_score = self.exp_avg_ipt[n_std] * self.exp_avg_unc[n_std]
        elif metric == "mag":
    ...
    def update_ipt(self, model):
        for n, p in model.named_parameters():
            if "lora_" in n and p.requires_grad and p.grad is not None:
                ...
                if n_std not in self.ipt:
                    print(f"Adding missing tracking dict entry for: {n_std}")
                    self.ipt[n_std] = torch.zeros_like(p)
                    self.exp_avg_ipt[n_std] = torch.zeros_like(p)
                    self.exp_avg_unc[n_std] = torch.zeros_like(p)

                with torch.no_grad():
                    # Calculate sensitivity
                    self.ipt[n_std] = (p * p.grad).abs().detach()
                    # Update sensitivity
                    self.exp_avg_ipt[n_std] = self.beta1 * self.exp_avg_ipt[n_std] + \
                                              (1 - self.beta1) * self.ipt[n_std]
                    # Update uncertainty
                    self.exp_avg_unc[n_std] = self.beta2 * self.exp_avg_unc[n_std] + \
                                              (1 - self.beta2) * (self.ipt[n_std] - self.exp_avg_ipt[n_std]).abs()
    # def update_ipt(self, model):
    #     for n,p in model.named_parameters():
    #         if "lora_" in n:
    ...
    def debug_lora_params(model):
        ...
        print(f"Found {len(lora_names)} LoRA parameters:")
    ...
    def pre_initialize_tracking_dicts(self, module, prefix=""):
        """Initialize tracking dictionaries for all LoRA parameters."""
        for name, param in module.named_parameters():
            if "lora_" in name:
                ...
                if full_name not in self.ipt:
                    # print(f"Pre-initializing tracking for: {full_name}")
                    self.ipt[full_name] = torch.zeros_like(param)
                    self.exp_avg_ipt[full_name] = torch.zeros_like(param)
                    self.exp_avg_unc[full_name] = torch.zeros_like(param)

    def update_and_mask(self, model, global_step):
        # Check if model is the full model or just image_encoder
        if hasattr(model, 'image_encoder'):
            image_encoder = model.image_encoder
        else:
            image_encoder = model
        if global_step < self.total_step - self.final_warmup:
            # Update importance scores element-wise with the right model
            self.update_ipt(image_encoder)

            # Budget schedule
        curr_rank, mask_ind = self.schedule_threshold(global_step)
        if mask_ind:
            # Mask to target budget using the right model
            mask_threshold = self.mask_to_target_rank(image_encoder, curr_rank)
        else:
            mask_threshold = None

        self._maybe_tb_writter_log(image_encoder)
